...
describe('Weapon', () => {
    ...
    describe('fire', () => {
        ...
        it('sets lastFireTime', () => {
            performance.now.mockReturnValue(500);
            weapon.fire();
            expect(weapon.lastFireTime).toBe(500);
        });
    ...
    describe('update', () => {
        it('completes reload after reloadTime', () => {
            ...
            performance.now.mockReturnValue(0);
            weapon.reload();
            performance.now.mockReturnValue(999);
            weapon.update();
            expect(weapon.isReloading).toBe(true);
            performance.now.mockReturnValue(1000);
            weapon.update();
    ...
    describe('getAmmoString', () => {
        it('returns ammo count', () => {
            expect(weapon.getAmmoString()).toBe('10/10');
        });

        it('returns RELOADING when reloading', () => {
            weapon.currentAmmo = 5;
            performance.now.mockReturnValue(0);
            weapon.reload();
            expect(weapon.getAmmoString()).toBe('RELOADING');
        });
    });
});
